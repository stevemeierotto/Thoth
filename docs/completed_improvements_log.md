...
Last updated: 2026-06-26 (C4 complete — dev tier + CI wiring)
...
- **Local:** `ctest -L fast` (~25s cognitive); `ctest -L pr` (full PR gate)
- **GitHub Actions:**
  - PR/push: `ctest -L pr`
...
- **Verify**: `cmake --build build/debug --target thoth-control-panel`; `ctest -L fast` (2/2; `test-suite-dev` + `reflection-ab-benchmark`).
...
- **Verify**: `ctest -L fast`; `./tests/thoth-unit-tests` (parallel retrieval test passes).
