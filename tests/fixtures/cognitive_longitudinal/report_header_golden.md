# Cognitive Longitudinal Report

## Reproducibility

- **protocol_version:** C6.3 v0.2.1
- **metric_schema_version:** 1.0
- **analyzer_version:** 0.2.0
- **analyzer_commit_hash:** golden
- **generated_at_ms:** 1800002000000 (2027-01-15T08:33:20Z)
- **report_completeness:** complete
- **evidence_scope:** exploratory_only
- **confidence_label:** low
- **window.start_ms:** 1797581800000
- **window.end_ms:** 1800001000000
- **window.days:** 28
- **window.anchor:** official_benchmark_execution
- **reproducibility.input_log_path:** tests/fixtures/cognitive_longitudinal/metrics.jsonl
- **reproducibility.benchmark_run_ids_consumed:** 8 (run-c601, run-c602, run-c603, run-c604, run-c606, run-c608, run-c610, run-c611)
- **threat_schema_version:** 1.0

**Observational evidence only.** This report describes statistical trends in immutable benchmark artifacts over a preregistered window. It does **not** establish causal relationships between code changes, configuration changes, and observed performance. Correlation in a rolling window is not proof of causation. Official learning claims require controlled benchmarks (E2, E3, B1, C3/C5) in addition to longitudinal evidence.
